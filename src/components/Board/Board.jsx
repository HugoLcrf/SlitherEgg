...
const Board = () => {
  ...
  const [showSubmit, setShowSubmit] = useState(false); // Etat pour le formulaire submit
  const [showScoreboard, setShowScoreboard] = useState(false); // Etat pour le scoreboard
  ...
  const replay = () => {
    ...
    // Réinitialiser l'affichage après le redémarrage
    setShowSubmit(false);
  ...
  useEffect(() => {
    if (gameOver && !hasEnteredResults) {
      setTimeout(() => setShowSubmit(true), 5000); // Afficher Submit après 3 secondes
      setTimeout(() => setShowScoreboard(true), 5000); // Afficher Scoreboard après 3 secondes
    }
  ...
  return (
    <>
      ...
      {gameOver && !hasEnteredResults && showSubmit && (
        <Submit
          score={score}
          death={death}
      ...
      {gameOver && showScoreboard && <Scoreboard />}
